...
> **Heads up:** the `module-template` referenced below is part of [Freshify](https://freshify.io)'s commercial offering, not the public foundation. The walkthrough is still useful as a *conceptual* tour of what a sovereign module contains — BE, FE, registry, settings page, SMI conformance — and the shape that anything you build should match. The three foundation modules ([`freshify-users`](https://github.com/freshifyv2/freshify-users), [`freshify-companies`](https://github.com/freshifyv2/freshify-companies), [`freshify-workspaces`](https://github.com/freshifyv2/freshify-workspaces)) are working reference implementations you can read against [`docs/smi-spec.md`](./smi-spec.md) — the SMI is complete and stable, you can build a sovereign module against it today without the template. If you'd rather skip the build and start from production-tested module starters, see [SUPPORT.md](../SUPPORT.md) or contact [Freshify](https://freshify.io).
